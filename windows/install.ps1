...
for ($i = 1; $i -le $Count; $i++) {
    ...
    Set-CredentialAcl -FilePath $ConfigEnv
    ...
    # Run under SYSTEM: no password to rotate, survives all logoff/logon, highest-privilege principal
    # the task scheduler allows without a stored-credential prompt.
    $TaskPrincipal = New-ScheduledTaskPrincipal `
        -UserId    'NT AUTHORITY\SYSTEM' `
        -LogonType  ServiceAccount `
        -RunLevel   Highest

    # Restart on failure: up to 3 times with a 30-second interval, then every 60 seconds thereafter.
    ...
    Register-ScheduledTask `
        ...
        -Description "Ephemeral GitHub Actions runner instance $i (gh-runner-windows@$i). Managed by install.ps1." | Out-Null
