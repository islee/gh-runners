#!/usr/bin/env bash
# bootstrap.sh — stable ENTRYPOINT trust root for the containerized "android" runner.
#
...
#
# WHY android is docker-only: there is no native android runner (the KVM emulator + SDK live in the
# image), so unlike light/supabase the only variant is this one. It still carries X-Fleet-Variant:
# docker → broker manifest key `android-docker` (uniform with the other containerized runners).
#
# Trust model (broker-anchored), fail-safe, and dependencies are identical to light/docker/bootstrap.sh
# — see that file for the full rationale. This is the android-flavored copy (own defaults + manifest
# subpath); the two are intentionally near-identical.
...
# Repo subpath holding this variant's managed files + manifest (raw.githubusercontent.com layout).
# android is its own top-level dir (no <type>/docker nesting like light/supabase).
readonly VARIANT_SUBPATH="android"
# Variant tag sent to the broker; composes the manifest key as <type>-<variant> (→ android-docker).
readonly FLEET_VARIANT="docker"
...
resolve_credential() {
  ...
  if [[ -n "${BROKER_URL:-}" ]]; then
    ...
    _resp="$(curl --silent --fail --max-time 15 -X POST \
      ...
      -H "X-Fleet-Variant: ${FLEET_VARIANT}" \
      "${BROKER_URL%/}/token")" || {
...
_try_update() {
  ...
  local _manifest_url="https://raw.githubusercontent.com/${UPDATE_REPO}/${_desired_ref}/${VARIANT_SUBPATH}/.fleet-manifest"
  ...
  _base_url="https://raw.githubusercontent.com/${UPDATE_REPO}/${_desired_ref}/${VARIANT_SUBPATH}"
