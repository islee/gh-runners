#!/usr/bin/env bash
# runner-payload.sh — swappable job-runner payload for the containerized "android" runner.
#
...
#
# WHY no emulator boot here: the Android emulator is booted/torn-down per job by the runner job
# hooks (ACTIONS_RUNNER_HOOK_JOB_{STARTED,COMPLETED}, wired in the Dockerfile), NOT by this payload.
# This script only registers the runner and runs one job; the hooks own the emulator lifecycle.
#
