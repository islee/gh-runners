#!/usr/bin/env bash
# self-update.sh — fleet-code updater for the "light" runner, invoked between jobs.
#
...
#   77 — update committed; caller (runner-loop.sh) should exit 0 so the supervisor relaunches
#        the new payload via runner-bootstrap.sh.
#
# Security model:
...
#   - Path allowlist: only hardcoded basenames are updatable; any / or .. in a manifest entry
#     aborts the entire update (not just that entry).
#   - Stage-all-then-commit: every staged file is verified (sha256) and syntax-checked (bash -n)
#     before any file is moved into place. A failed check leaves current code untouched.
#   - Atomic commit: snapshot current → last_good/, mv staged files, write version stamp LAST.
#     Interrupted before the stamp means "stale/absent stamp" → retried next cycle, never partial.
#   - Fail-safe: ANY unexpected error returns 0 (no update). Current code is always preserved.
#
# CRITICAL: runner-bootstrap.sh is NOT in the allowlist and must NEVER be added. It is the
# recovery path — if it were updatable, a bad update could brick its own rollback mechanism.
...
readonly RUNNER_TYPE="light"

# ── Config knobs (sourced from config.env; defaults below) ────────────────────
...
# ── Args (from runner-loop.sh after broker /token response) ───────────────────
...
# runner-bootstrap.sh, config.env, secret.env, .runner, .credentials* are NEVER added here.
...
# Compute sha256 of a file. Portable: uses sha256sum (Linux) or shasum -a 256 (macOS).
_sha256_file() {
...
# Check if a basename is in UPDATE_ALLOWLIST_CSV.
_in_allowlist() {
  local _f="$1" _af
...
# Write has_pending_swap=1 to .fleet-state (preserving existing starts count).
# Called after all staged files are moved — tells bootstrap to watch for crash-loops.
_set_pending_swap() {
...
# Get file permission bits as an octal string (uses python3, a runner prerequisite).
_get_file_mode() {
  python3 -c "
...
_do_update() {
  ...
  # ── 1. Gate: AUTO_UPDATE, dormant check, interval ─────────────────────────
  ...
  # Record check time before fetch so a network failure doesn't cause tight retry loops.
  printf '%s\n' "${_now}" > "${TS_FILE}"
  ...
  # IMPORTANT: the expected hash came from the broker (out-of-repo trust anchor), not the repo.
  # A match proves the manifest was not tampered with after the broker's snapshot.
  local _fetched_hash
  ...
  # ── 4. Already current? (compare by manifest content-hash, not version string) ──
  # WHY content-hash: a maintainer reverting a bad release by moving the tag backward still
  # propagates — a version-string-only check would think the reverted code is already current.
  ...
  while IFS= read -r _line; do
    ...
    # Parse "<sha256>  <basename>" (two spaces, matching sha256sum/shasum output format).
    read -r _hash _fname <<< "${_line}"

    # CRITICAL path safety: reject traversal characters and off-allowlist basenames immediately.
    # The ENTIRE update is aborted — not just the offending entry — to prevent partial poisoning.
  ...
  for _i in "${!_entry_names[@]}"; do
    ...
    # SELFTEST: invoke runner-loop.sh in dry-run mode to verify required functions are defined.
    # Guards: only for runner-loop.sh (self-update.sh is the invoker — circular); ENABLE_SELFTEST=1.
  ...
  # ── 8. Atomic commit ──────────────────────────────────────────────────────
  # a. Snapshot current managed files to last_good/ (safety net for bootstrap rollback).
  ...
  # b. Mark pending swap so bootstrap watches for crash-loops on the new code.
  _set_pending_swap

  # c. Move staged files into place (atomic rename within same filesystem).
  for _i in "${!_entry_names[@]}"; do
    _fname="${_entry_names[${_i}]}"
    local _staged="${STAGING_DIR}/${_fname}"
    local _target="${RUNNER_DIR}/${_fname}"
    # Preserve existing mode; default 755 for new scripts.
    local _mode="755"
  ...
  # d. Reconcile deletions: remove allowlist files absent from the new manifest.
  for _af in ${UPDATE_ALLOWLIST_CSV}; do
  ...
  # e. Update local manifest copy.
  cp "${_manifest_tmp}" "${LOCAL_MANIFEST}"
  rm -f "${_manifest_tmp}"
  rm -rf "${STAGING_DIR}"

  # f. Write version stamp LAST — this is the commit point. If interrupted before this step,
  #    the stamp is stale/absent → the update retries next cycle. Never a partial cross-file state.
...
# Propagate UPDATE_APPLIED (77) so runner-loop.sh exits 0 and the supervisor relaunches bootstrap.
# All other non-zero exits are unexpected errors; runner-loop.sh logs and continues normally.
exit "${_update_exit}"
