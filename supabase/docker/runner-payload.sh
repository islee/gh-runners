#!/usr/bin/env bash
# runner-payload.sh — swappable job-runner payload for the containerized "supabase" runner.
#
# Purpose: register this runner as ephemeral and exec run.sh for exactly one job. On exit,
# docker-compose's `restart: always` restarts the container to re-register.
#
...
#
# WHY --work RUNNER_WORKDIR (unlike light/android): this is a docker-OUT-of-docker runner — the job
# runs `supabase start` against the HOST daemon via the mounted socket. The work dir is bind-mounted
# at the SAME absolute path host↔container so the host daemon resolves job bind-mounts by host path.
#
...
# Work dir bind-mounted at the SAME absolute path host↔container (see docker-compose.yml volumes).
RUNNER_WORKDIR="${RUNNER_WORKDIR:-_work}"

# ── Credential resolution ──────────────────────────────────────────────────────
# Priority: REG_TOKEN (from bootstrap) → RUNNER_TOKEN (A) → BROKER_URL+BROKER_SECRET (B)
#           → ACCESS_TOKEN (PAT). Sets REG_TOKEN (the token passed to config.sh).
resolve_credential() {
  if [[ -n "${REG_TOKEN:-}" ]]; then
