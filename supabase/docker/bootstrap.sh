#!/usr/bin/env bash
# bootstrap.sh — stable ENTRYPOINT trust root for the containerized "supabase" runner.
#
...
# GitHub ref, then exec the payload. Each container restart is the update boundary.
#
...
#
# This is the supabase-docker flavor of light/docker/bootstrap.sh — see that file for the full
# trust-model / fail-safe rationale. Differences here: own defaults + manifest subpath, and it
# carries X-Fleet-Variant: docker → broker manifest key `supabase-docker` (distinct from the
# native `supabase` key, since the docker payload differs from the systemd runner-loop.sh).
...
RUNNER_NAME="${RUNNER_NAME:-gh-runner-supabase-${OWNER:-$(hostname -s)}-${RUNNER_NUMBER:-1}}"
...
readonly VARIANT_SUBPATH="supabase/docker"
# Variant tag sent to the broker; composes the manifest key as <type>-<variant> (→ supabase-docker).
