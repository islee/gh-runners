#!/usr/bin/env bash
# runner-bootstrap.sh — stable supervisor entrypoint for the iOS/Android-on-Mac runner.
#
# ProgramArguments target for com.example.gh-runner.plist (launchd). Maintains a crash counter
# and performs rollback if runner-loop.sh crashes repeatedly after a fleet-code swap, then
# exec's the payload.
#
...
#
# State persists in .fleet-state (key=value, no eval) so it survives launchd restarts.

set -euo pipefail

# ---------------------------------------------------------------------------
# Bootstrap — config.env sits next to this script (its runner dir)
# ---------------------------------------------------------------------------
...
# CRITICAL: this list must match self-update.sh UPDATE_ALLOWLIST_CSV exactly.
...
# WHY exec: replaces this process with the payload so launchd tracks runner-loop.sh's PID
# directly. SIGTERM from launchd bootout reaches runner-loop.sh cleanly.
